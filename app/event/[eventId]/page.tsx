...
export default function JoinPage({ params }: { params: { eventId: string } }) {
  ...
  const verifyEdit = async () => {
    if (!editName.trim() || !editPw) { setEditError('名前とパスワードを入力してください'); return }
    setEditError('')
    const res = await fetch('/api/participants/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ eventId: params.eventId, displayName: editName.trim(), password: editPw }),
    })
    ...
    try {
      ...
      if (isEditMode && participantId) {
        res = await fetch(`/api/participants/${participantId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ password: pw, slots: [...mySlots] }),
        })
      } else {
  ...
  return (
    <div style={{ maxWidth:860, margin:'0 auto', padding:'32px 24px 64px' }}>
      {isEditMode && (
        <div style={{ background:'linear-gradient(135deg,var(--teal),var(--teal2))', color:'white', padding:'10px 16px', borderRadius:'var(--r-sm)', fontSize:13, marginBottom:16, display:'flex', gap:8 }}>
      ...
      {mode === 'form' && (
        <div className="card" style={{ marginBottom:16 }}>
          <div className="field">
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="名前を入力" maxLength={40} onKeyDown={e => e.key === 'Enter' && startInput()} />
          </div>
          <div className="field" style={{ marginBottom:0 }}>
            <label>パスワード（任意）</label>
            <input type="password" value={pw} onChange={e => setPw(e.target.value)} placeholder="後で再編集したい場合は設定" />
            <div style={{ fontSize:12, color:'var(--muted)', marginTop:4 }}>パスワードを設定すると後から内容を変更できます</div>
          </div>
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginTop:16 }}>
            ...
            <button className="btn btn-outline btn-sm" onClick={() => router.push(`/event/${params.eventId}/results`)}>集計を見る</button>
          </div>
        </div>
      )}
      ...
      {mode === 'reauth' && (
        <div className="card" style={{ marginBottom:16 }}>
          ...
          <div className="field">
            <label>登録時の名前</label>
            <input type="text" value={editName} onChange={e => setEditName(e.target.value)} placeholder="名前を入力" maxLength={40} />
          </div>
          <div className="field" style={{ marginBottom:0 }}>
            <label>パスワード</label>
            <input type="password" value={editPw} onChange={e => setEditPw(e.target.value)} placeholder="登録時のパスワード" />
          </div>
          ...
          <div style={{ display:'flex', gap:10, marginTop:16 }}>
            ...
            <button className="btn btn-outline btn-sm" onClick={() => { setMode('form'); setEditError('') }}>キャンセル</button>
          </div>
        </div>
      )}
          ...
          <div className="card" style={{ marginBottom:16 }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8, marginBottom:12 }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button className="btn btn-outline btn-sm" onClick={() => setMySlots(new Set())}>クリア</button>
