...
export default function ResultsPage({ params }: { params: { eventId: string } }) {
  ...
  const rankedGroups = (() => {
    ...
    event.dates.forEach(d => {
      ...
      s.forEach(sl => { slotCount[sl] = 0; slotNames[sl] = []; allSlots.push(sl) })
    })
    participants.forEach(p => {
      p.slots.forEach(s => {
        if (slotCount[s] !== undefined) { slotCount[s]++; slotNames[s].push(p.display_name) }
      })
    })
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      const [pd, pt] = prev.split('T'), [cd, ct] = cur.split('T')
      if (pd === cd && toMin(ct) === toMin(pt) + 30 && slotCount[cur] === slotCount[prev]) {
        gPrev = cur
      } else {
        const slots = sorted.filter(s => s >= gStart && s <= gPrev)
        const names = [...new Set(slots.flatMap(s => slotNames[s]))]
        const [, st] = gStart.split('T'), [, et] = gPrev.split('T')
        const em = toMin(et) + 30
        const endStr = `${String(Math.floor(em/60)).padStart(2,'0')}:${String(em%60).padStart(2,'0')}`
        groups.push({ date: gStart.split('T')[0], start: st, end: endStr, count: slotCount[gStart], names })
        gStart = cur; gPrev = cur
      }
    }
    const slots2 = sorted.filter(s => s >= gStart && s <= gPrev)
    const names2 = [...new Set(slots2.flatMap(s => slotNames[s]))]
    const [, st2] = gStart.split('T'), [, et2] = gPrev.split('T')
    const em2 = toMin(et2) + 30
    groups.push({ date: gStart.split('T')[0], start: st2, end: `${String(Math.floor(em2/60)).padStart(2,'0')}:${String(em2%60).padStart(2,'0')}`, count: slotCount[gStart], names: names2 })
  ...
  const rankedText = (() => {
    ...
    rankedGroups.forEach(g => { if (!byCount[g.count]) byCount[g.count] = []; byCount[g.count].push(g) })
  ...
  const copyCommon = async () => {
    ...
    setCopiedCommon(true); setTimeout(() => setCopiedCommon(false), 2000)
  }
  const copyRanked = async () => {
    if (!rankedText) return
    await navigator.clipboard.writeText(rankedText)
    setCopiedRanked(true); setTimeout(() => setCopiedRanked(false), 2000)
  }
  ...
  if (!event) return <div style={{ padding:32, color:'var(--muted)' }}>読み込み中...</div>
      ...
      {tab === 'common' && (
        <>
          <div className="card" style={{ marginBottom:16 }}>
            ...
            <label style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, fontSize:13, color:'var(--muted)', cursor:'pointer' }}>
              <input type="checkbox"
                checked={checkedIds.size === participants.length}
                onChange={e => setCheckedIds(e.target.checked ? new Set(participants.map(p => p.id)) : new Set())}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {participants.map((p, i) => (
                <label key={p.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', borderRadius:'var(--r-sm)', background:'var(--cream)', cursor:'pointer', fontSize:14 }}>
                  <input type="checkbox" checked={checkedIds.has(p.id)}
                    onChange={e => setCheckedIds(prev => { const n = new Set(prev); e.target.checked ? n.add(p.id) : n.delete(p.id); return n })}
                    style={{ accentColor:'var(--teal)' }}
                  />
            ...
            {commonSlots.length === 0 ? (
              ...
              <>
                <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:16 }}>
                  {commonSlots.map((r, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:'white', border:'1px solid var(--border)', borderRadius:'var(--r-sm)' }}>
                      <div style={{ fontFamily:'"DM Serif Display",serif', fontSize:'1.4rem', color:'var(--teal)', minWidth:36, textAlign:'center' }}>{r.start.split(':')[0]}</div>
                      <div>
            ...
            <>
              <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:16 }}>
                {rankedGroups.map((g, i) => {
                  ...
                  return (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:'white', border:'1px solid var(--border)', borderRadius:'var(--r-sm)' }}>
                      <div style={{ fontFamily:'"DM Serif Display",serif', fontSize:'1.4rem', color: g.count === participants.length ? 'var(--teal)' : 'var(--muted)', minWidth:36, textAlign:'center' }}>{g.count}</div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontWeight:500, fontSize:14 }}>{fmtDate(g.date)} {g.start}〜{g.end}</div>
                        <div style={{ background:'var(--border)', borderRadius:4, height:4, margin:'4px 0' }}>
                          <div style={{ background:'var(--teal)', height:4, borderRadius:4, width:`${pct}%`, transition:'width 0.3s' }} />
                        </div>
                        <div style={{ fontSize:12, color:'var(--muted)' }}>
                          {g.names.slice(0, 5).join('、')}{g.names.length > 5 ? `他${g.names.length - 5}名` : ''}
                        </div>
                      </div>
                      <div style={{ fontSize:12, color:'var(--muted)', whiteSpace:'nowrap' }}>{g.count}/{participants.length}人</div>
                    </div>
                  )
                })}
              </div>
